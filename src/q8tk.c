...
static	void	combo_event_entry_activate( Q8tkWidget *dummy,
					    Q8tkWidget *parent )
{
  widget_signal_do( parent, "activate" );
}
...
static void fsel_selection_view_callback( Q8tkWidget *dummy, void *fselect )
{
...
Q8tkWidget	*q8tk_file_selection_new( const char *title, int select_ro )
{
  ...
  q8tk_widget_show( hbox );
  {
	...
	q8tk_widget_show( wk );
								...
								/* [VIEW] */
      fselect->stat.fselect.view_button = q8tk_button_new_with_label(" VIEW ");
 ...
 *  ・テキストファイルの判断は、拡張子のみ
 *    SJIS か EUC かを曖昧に判断。しかもTABは展開されない
...
static	void	q8tk_utility_view_end( Q8tkWidget *dummy_0, void *dummy_1 );
int		q8tk_utility_view( const char *filename )
{
  ...
  if( file_text == FALSE ){		/* バイナリファイル */
    ...
    file_code = Q8TK_KANJI_ANK;
					...
					/* ファイル先頭 1KB を適当に見て  */
    int	i, sz, sjis = 0, euc = 0;	/* 漢字コードを判定。ほんとに適当 */
	...
	}else if( 0x81 <= buf[i]  &&  buf[i] <= 0x9f ){
	  file_code = Q8TK_KANJI_SJIS;
	  break;

	}else if( buf[i]==0xfe || buf[i]==0xfd ){
	  file_code = Q8TK_KANJI_EUC;
	  break;

	}else{
	  if( i<sz ){
	    if( buf[i+1]==0xfe || buf[i+1]==0xfd ){
	      file_code = Q8TK_KANJI_EUC;
	      break;
	    }
	    if( 0xa1 <= buf[i]    &&  buf[i] <= 0xfe   &&
		0xa1 <= buf[i+1]  &&  buf[i+1] <= 0xfe ){
	      file_code = Q8TK_KANJI_EUC;
	      break;
	    }
	    i++;
  ...
  {						/* それにボックスを乗せる */
    ...
    {							/* 終了ボタンを配置 */
      ...
      q8tk_accel_group_add( view_widget_accel, Q8TK_KEY_ESC, b, "clicked" );
    }
  }
